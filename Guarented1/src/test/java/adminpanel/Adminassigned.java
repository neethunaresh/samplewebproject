...
public class Adminassigned extends TestBase2 {
	@BeforeMethod
	...
	public void adminflowthroughmyorders() throws Exception {
		...
		// System.out.println("after change of special char                "+
		// result);
		...
		// System.out.println("1st page of mobile number                " +
		// mobilenumber);
		...
		List<WebElement> listdatainadmin = driver
				.findElements(By
						.xpath("//*[@id='content']/md-content/md-table-container/table/tbody/tr[1]/td"));
		System.out.println("list in adminpanel " + listdatainadmin.size());
		for (int k = 0; k < listdatainadmin.size(); k++) {
			System.out.println("data  in admin panel" + " " + k + " " + "is :"
					+ listdatainadmin.get(k).getText());
		}
		...
		Assert.assertTrue(final11.contains(vasi1),
				"ids are not same in staging and admin");
		System.out.println("ids are  same in staging and admin");
		Assert.assertTrue(vasi2.contains(srinivas),
				"email ids are  not same in both admin and staging");
		System.out.println("email ids are  same in staging and admin");
		Assert.assertTrue(vasi2.contains(afterreplacemobilenumber),
				"mobile numbers are  not same in both admin and staging");
		System.out.println("mobile numbers  are  same in staging and admin");
		Assert.assertTrue(vasi2.contains(data3),
				"names are  not same in both admin and staging");
